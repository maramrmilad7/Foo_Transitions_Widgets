import 'package:flutter/material.dart';
import 'package:footransitionswidgetsapp/widgets/navbutton.dart';

class HomePage extends StatelessWidget {
  const HomePage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'Essential Page Transitions',
          style: TextStyle(color: Colors.white),
        ),
        backgroundColor: Colors.blue,
        centerTitle: true,
      ),
      body: Container(
        width: double.infinity,
        decoration: BoxDecoration(
          gradient: LinearGradient(
            begin: Alignment.topLeft,
            end: Alignment.bottomRight,

            colors: [Colors.blue.withValues(alpha: 0.1), Colors.purple],
          ),
        ),
        child: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.symmetric(vertical: 20),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(Icons.animation, size: 80, color: Colors.blue.shade600),
                SizedBox(height: 16),
                Text(
                  'Most Used Transitions',
                  style: Theme.of(context).textTheme.headlineMedium?.copyWith(
                    color: Colors.blue.shade600,
                    fontWeight: FontWeight.bold,
                  ),
                ),
                SizedBox(height: 8),
                Text(
                  'Industry-standard page animations',
                  style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                    color: Colors.grey.shade600,
                    fontWeight: FontWeight.w400,
                  ),
                ),
                SizedBox(height: 30),
                NavButton(
                  routeName: 'fade',
                  label: 'Fade Transition',
                  icon: Icons.blur_on,
                  color: Colors.blue,
                ),
                NavButton(
                  routeName: 'slideFromRight',
                  label: 'Slide Right (iOS Style)',
                  icon: Icons.arrow_forward,
                  color: Colors.grey,
                ),
                NavButton(
                  routeName: 'slideFromLeft',
                  label: 'Slide Left (back nav)',
                  icon: Icons.arrow_back,
                  color: Colors.orange,
                ),
                NavButton(
                  routeName: 'slideFromBottom',
                  label: 'Slide Up',
                  icon: Icons.arrow_upward,
                  color: Colors.green,
                ),
                NavButton(
                  routeName: 'scalepage',
                  label: 'Scale  (Material Design)',
                  icon: Icons.zoom_out_map,
                  color: Colors.purple,
                ),
                 NavButton(
                  routeName: 'rotationpage',
                label: 'Rotation (Creative)',
                  icon: Icons.rotate_right,
                  color: Colors.blue,
                ),
                 NavButton(
                  routeName: 'slideandfadepage',
                label: 'Slide And Fade ',
                  icon: Icons.layers,
                  color: Colors.pink,
                ),
                 NavButton(
                  routeName: 'zoompage',
                label: 'Zoom  (Hero Style)',
                  icon: Icons.zoom_in,
                  color: Colors.teal,
                ),
                 NavButton(
                  routeName: 'notransition',
                label: 'No Transition',
                  icon: Icons.flash_on,
                  color: Colors.blue,
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }
}

